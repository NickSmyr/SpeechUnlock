...
### Interactive tutorial for demo taken from
...
def play_wav(wav, sample_rate , blocking=True):
    try:
        import sounddevice as sd

        # Small bug with sounddevice.play: the audio is cut 0.5 second too early. We pad it to
        # make up for that
        wav = np.concatenate((wav, np.zeros(sample_rate // 2)))
        sd.play(wav, sample_rate, blocking=blocking)
    except Exception as e:
        print("Failed to play audio: %s" % repr(e))


def plot_similarity_matrix(
    matrix, labels_a=None, labels_b=None, ax: plt.Axes = None, title=""
):
    if ax is None:
        _, ax = plt.subplots()
    fig = plt.gcf()
    ...
    img = ax.matshow(
        matrix, extent=(-0.5, matrix.shape[0] - 0.5, -0.5, matrix.shape[1] - 0.5)
    )
...
def interactive_diarization(
    similarity_dict, wav, wav_splits, sample_rate ,x_crop=5, show_time=False
):
    fig, ax = plt.subplots()
    ...
    play_wav(wav, sample_rate, blocking=False)
    plt.show()
